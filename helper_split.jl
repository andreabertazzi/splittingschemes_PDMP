...
function Vrates_first(x,v)
    pos_part(v[1] * Vprime(x[1]-x[2]))
end


# function define_Vrates(Vprime::Function, N::Integer)
#   if N > 3
#     Vrates = Vector{Function}(undef, N)
#     Vrates(x,v)[1] = pos_part(v[1] * Vprime(x[1]-x[2]))
#   for i = 2 : (N-1)
#       Vrates(x,v)[i] = pos_part(v[i] * ( Vprime(x[i]-x[i+1]) - Vprime(x[i-1]-x[i]) ))
#     end
#     Vrates(x,v)[N] = pos_part(v[N] * Vprime(x[N-1]-x[N]))
#   else
#     error("Must increase number of particles.")
#   end
#   Vrates
# end
...
function define_Wrates(Wprime::Function, x::Vector{<:Real}, v::Vector{<:Real}, i::Integer, j::Integer, N::Integer)
  if (i <= N) && (j <= N-1)
    ...
    return pos_part(v[i] * Wprime(x[i]-x[set_indeces[j]])) / N
  else
    error("Index larger than number of particles")
  end
end