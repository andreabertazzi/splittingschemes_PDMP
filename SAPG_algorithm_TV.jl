...
#}
function SAPG_algorithm_TV(y, warmupSteps,lambda,gamma,total_iter,eta_init,min_eta,max_eta,d_exp,d_scale,gradF,f,burnIn,stopTol)
    dimX=length(y);
    g=x->TVnorm(x);
    chambolleit = 25;
    proxG = (x,lambd,theta) -> chambolle_prox_TV(x,lambd*theta,chambolleit); # function handle for proximal operator of g(x)
    #logPi = (x,theta)->-f(x)-theta*g(x)
    ##### MYULA sampler
    #### SAPG algorithm 1
    # We work on a logarithmic scale, so we define an axiliary variable 
    # eta such that theta=exp{eta}. 
    # delta(i) for SAPG algo 
    delta = i-> d_scale*( (i^(-d_exp)) /dimX ); 
    ### MYULA Warm-up
    X_wu = y;
    if(warmupSteps>0)
        # Run MYULA sampler with fix theta to warm up the markov chain
        fix_theta=eta_init;
        #logPiTrace_WU=zeros(warmupSteps);  #logPiTrace to monitor convergence
        ...
        for ii = 2:warmupSteps
            ...
            #Display Progress
            if mod(ii, round(warmupSteps / 100)) == 0
                print(string("\b\b\b\b",round(ii / (warmupSteps) * 100),"\r"));
            end
        end
        #results.logPiTrace_WU=logPiTrace_WU;
    end
    ## Run SAPG algorithm 1 to estimate theta
    theta=zeros(total_iter);
    theta[1]=eta_init;
    eta=zeros(total_iter);
    eta[1]=eta_init;
    #logPiTraceX=zeros(total_iter);# to monitor convergence
    #gX=zeros(total_iter); # to monitor how the regularisation function evolves
    ...
    for ii = 2:total_iter 
        ...
        if mod(ii, round(total_iter / 100)) == 0
            print(string("\b\b\b\b",round(ii / (total_iter) * 100), "\r"));
        end
        global last_samp=ii;
        #Check stop criteria. If relative error is smaller than op.stopTol stop
        relErrTh1=abs(exp(mean(eta[burnIn:ii]))-exp(mean(eta[burnIn:ii-1])))/exp(mean(eta[burnIn:ii-1]));
        if(relErrTh1<stopTol) && (ii>burnIn+1)
            break;
        end
    ...
    #print(string(last_samp,"\n"))
    #print(size(eta))
    theta_EB = exp(mean(eta[burnIn:last_samp]));
