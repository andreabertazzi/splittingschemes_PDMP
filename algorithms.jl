...
function draw_event_times_lipschitzgrad(x::Vector{Float64},v::Vector{Int64},grad_pot::Function,L::Float64)
    dim = length(x)
    (a,b) = give_coeffs_events_lipschitzgrad(x,v,grad_pot,L)
    # b = L * sqrt(dim) 
    # a = max.(0,v.*grad_pot(x))
    proposed_switches = switchingtime.(a,b)
    final_rates = a .+ proposed_switches .* b
...
function ZigZag(∇E::Function, get_coeffs::Function, nr_proposals::Integer, dim::Integer, x_init::Vector{Float64} = Vector{Float64}(undef,0),
    ...
    #   (Δt_proposed_switches,end_rates) = draw_event_times(x,v)
    #   Δt_proposed_switches = switchingtime.(a,b)
      if (excess_rate == 0.0)
          Δt_excess = Inf
      else
      ...
      while (!finished)
        ...
        #   a = a .+ b * Δt; # O(d)
          ...
          if (Δt_switch_proposed < Δt_excess)
              switch_rate = v[i] * ∂E(i,x)
              proposedSwitchIntensity = a[i] + b * Δt
            #   println("Proposed intensity is $proposedSwitchIntensity, true rate is $switch_rate, proposed time is $Δt_switch_proposed")
            #   proposedSwitchIntensity = end_rates[i]
              if proposedSwitchIntensity < switch_rate
                  println("ERROR: Switching rate exceeds bound.")
                  println(" simulated rate: ", proposedSwitchIntensity)
                  println(" actual switching rate: ", switch_rate)
      ...
      totswitch = accepted_switches + rejected_switches
      println("Nr of total proposals is $totswitch")
      fracswitch = accepted_switches / totswitch
      println("Fraction of accepted switches is $fracswitch")
      return skel_chain
